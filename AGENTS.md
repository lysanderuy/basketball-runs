...
- **Never hard-delete `queue_entries` with game history.** Always set `status = 'removed'` instead.
...
```bash
# Schema change:
npm run db:generate        # generates SQL → supabase/migrations/
npm run db:push            # supabase db push — deploys all pending migrations
...
- Never run `drizzle-kit migrate` — it bypasses Supabase migration history
- Never edit a migration file after it has been applied — write a new one
- Drizzle generates table/index/FK SQL only — RLS and triggers are always hand-written
...
- All schemas in `lib/validations/index.ts`
- Always export the inferred type alongside the schema
- Enum values must match Drizzle enum values exactly

---

## Supabase Realtime

- Subscriptions belong in `hooks/` only
- Subscribe to `games` for live score updates
...
- Do not run `drizzle-kit migrate` — use `supabase db push`
